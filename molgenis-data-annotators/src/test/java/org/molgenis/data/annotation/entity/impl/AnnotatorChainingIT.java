	...
	public void chain() throws IOException
	{
		...
		try (VcfRepository repo = new VcfRepository(vcf, "vcf"))
		{
					...
					"org.molgenis.data.annotation"))
			{
				...
				gonlAnnotator.getCmdLineAnnotatorSettingsConfigurer().addSettings("src/test/resources/gonl", -1);

				RepositoryAnnotator tgAnnotator = annotators.get("thousandGenomes");
				tgAnnotator.getCmdLineAnnotatorSettingsConfigurer().addSettings("src/test/resources/1000g", -1);
